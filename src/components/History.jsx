...
const History = () => {
  ...
  const [filterDate, setFilterDate] = useState(""); // independent history date filter
  const entriesPerPage = 10;

  // Filter history independently
  const filteredHistory = useMemo(() => {
    return filterDate ? history.filter((h) => h.date === filterDate) : history;
  }, [history, filterDate]);
  ...
  return (
    <div style={{ marginTop: "20px", border: "1px solid gray", padding: "10px", borderRadius: "8px", backgroundColor: "#333" }}>
      <h3>History</h3>
        ...
        <>
          <ul style={{ listStyle: "none", padding: 0 }}>
            {currentEntries.map((entry, idx) => (
              <li key={idx} style={{ marginBottom: "5px", padding: "5px", background: "#444", borderRadius: "4px" }}>
