...
const SummaryBox = () => {
  ...
  const adjustWallet = (type) => {
    ...
    if (!isNaN(value)) {
      const newWallet =
        type === "increase" ? wallet + value : wallet - value;

      setWallet(newWallet);

      addHistory(
        `${type === "increase" ? "Added" : "Removed"} ¥${value} ${
          type === "increase" ? "to" : "from"
        } wallet`,
  ...
  return (
    ...
    >
      ...
      <div style={{ color: "#4caf50", marginBottom: "5px" }}>
        Available: ¥{formatPrice(Math.round(displayWallet))}
      </div>

      <div style={{ color: "#f44336", marginBottom: "5px" }}>
        Expenses: ¥{formatPrice(Math.round(displayExpenses))}
      </div>

      <div style={{ color: "#2196F3", marginBottom: "10px" }}>
        Remaining: ¥{formatPrice(Math.round(remainingAmount))}
      </div>
